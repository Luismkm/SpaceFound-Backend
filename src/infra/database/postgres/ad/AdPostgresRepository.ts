...
export class AdPostgresRepository implements ICreateAdRepository, IFindAdsByAccountRepository {
  async create(params: CreateAdRepository.Params): Promise<CreateAdRepository.Result> {
    const { id, accountId, accountType, title, description, createdAt, serviceId } = params;
    const adCreated = await knexHelper.knex('ad').insert({ id, account_id: accountId, account_type: accountType, title, description, service_id: serviceId, created_at: createdAt }).returning('id');
